  ...
  const handleOAuthSignUp = async (provider: 'google' | 'microsoft' | 'apple') => {
    ...
    try {
      ...
      if (errorMessage.includes('not configured') || errorMessage.includes('client_id')) {
        errorMessage = `${provider.charAt(0).toUpperCase() + provider.slice(1)} sign-up is not configured yet. Please use email/password registration or contact support.`;
      }
      
      setError(errorMessage || `Failed to sign up with ${provider}. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 shadow-2xl">
            ...
            <div className="text-center mb-3">
              <h1 className="text-white text-xl mb-1">Join Nava AI</h1>
              <p className="text-gray-300 text-sm">Start creating with AI today</p>
            </div>
            ...
            <div className="mt-6 text-center space-y-3">
              <p className="text-gray-400 text-sm">
                ...
                <button onClick={onNavigateToLogin} className="text-white hover:underline">Sign in</button>
              </p>
